...
def main():
    ...
    args.save_subdir = dp_prefix + f"entropy_deltav_baseline_{args.preserve_rate1}_{args.preserve_rate2}_lam{args.lam1}_{args.lam2}_grid{args.grid_size}_lr{args.lr}_log{args.logspace}_da"
    ...
    train_ori_loss_list, train_entropy_loss_list, val_ori_loss_list, val_entropy_loss_list = [], [],[],[]
    train_ori_acc_list, train_entropy_acc_list, val_ori_acc_list, val_entropy_acc_list = [], [],[],[]
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_total_loss, train_ori_loss, train_entropy_loss, train_ori_acc, train_entropy_acc = train(args, train_loader, model, criterion, optimizer, epoch)

        # evaluate on validation set
        val_total_loss, val_ori_loss, val_entropy_loss, val_ori_acc, val_entropy_acc = validate(args, val_loader, model, criterion, epoch)
        ...
        train_entropy_loss_list.append(train_entropy_loss), val_entropy_loss_list.append(val_entropy_loss)

        train_ori_acc_list.append(train_ori_acc), val_ori_acc_list.append(val_ori_acc)
        train_entropy_acc_list.append(train_entropy_acc), val_entropy_acc_list.append(val_entropy_acc)

        draw_acc_loss_figure(args, os.path.join(args.result_path, "loss_acc.png"),
                             train_total_loss_list, val_total_loss_list,
                             train_ori_loss_list, val_ori_loss_list,
                             train_entropy_loss_list, val_entropy_loss_list,
...
def get_entropy(output):
    """
    Input:
        output: (N,class_number) tensor
    Return:
        entropy: scalar tensor
    """
    p = F.softmax(output, dim=1) # (N,class_number)
    logp = F.log_softmax(output, dim=1) # (N,class_number)
    entropy = -torch.sum(p * logp, dim=1)
    entropy = entropy.mean() # average across the mini-batch
    return entropy
...
def train(args, train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        entropy_loss = -entropy * args.lam2

        entropy_loss.backward()
        optimizer.step()
        loss = ori_loss.item() + entropy_loss.item()

        # measure accuracy and record loss
        ori_acc = accuracy(output.data, target)[0]
        entropy_acc = accuracy(delta_v.data, target)[0]
        ...
        entropy_losses.update(entropy_loss.item(), input.size(0)) # input.size(0) is the batch size

        ori_accs.update(ori_acc.item(), input.size(0))
        entropy_accs.update(entropy_acc.item(), input.size(0))
        ...
        if i % args.print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'entropy loss {entropy_losses.val:.4f} ({entropy_losses.avg:.4f})\t'
                  'ori acc {ori_accs.val:.3f} ({ori_accs.avg:.3f})\t'
                  'entropy acc {entropy_accs.val:.3f} ({entropy_accs.avg:.3f})'.format(
                      epoch, i, len(train_loader), batch_time=batch_time,
                      losses=losses, ori_losses=ori_losses, entropy_losses=entropy_losses,
...
def save_list(path, train_total_loss_list, val_total_loss_list,
    ...
    training_stats = {
        ...
        'train_entropy_loss': train_entropy_loss_list,
        'val_entropy_loss': val_entropy_loss_list,
        'train_ori_acc': train_ori_acc_list,
        'val_ori_acc': val_ori_acc_list,
        'train_entropy_acc': train_entropy_acc_list,
        'val_entropy_acc': val_entropy_acc_list,
    }
...
def draw_acc_loss_figure(args, path, train_total_loss_list, val_total_loss_list,
    ...
    plt.plot(x, train_entropy_acc_list, color="red", label="train entropy acc")
    plt.plot(x, val_entropy_acc_list, color="red", linestyle="dashed",label="val entropy acc")
    ...
    plt.plot(x, train_entropy_loss_list, color="red", label="train entropy loss")
    plt.plot(x, val_entropy_loss_list, color="red", linestyle="dashed", label="val entropy loss")
